// ****************************************************************************
//
// TractorBot code for Pi Wars 2014 (c) Phil Willis
//
//
//         */* Ipswich Maker Space Entry */*
//
// Build:
//	 g++ -std=c++0x -O0 -g3 -Wall -c -fmessage-length=0 -fpermissive -MMD -MP -MF"TractorBot.d" -MT"TractorBot.d" -o"TractorBot.o" "TractorBot.cpp"
//   g++ -L/usr/local/lib -o"TractorBot"  ./TractorBot.o -lpthread -lm -lwiringPi -lcwiid
//
//	Depends:
...
// Green button and LED  for human feed back 
#define GRNBUTTON 		1
#define GRNLED 			4
...
// Direction enum ( Does what its says on the tin )
...
// Basic motor speed limits 
const int highSpeed = 255;
const int lowSpeed = 74;

// Encoder mm per tick
const double mmPerTick =  5;

// Motor balance constant (TODO)
const double motorBalance = 1;
...
// Turn on or off a specific led
void setLed(int led,  int l1 )
{
	digitalWrite (led, l1);	
}
...
// Right turn with only one side powered
...
//Left turn with only one side powered
...
// Right turn with both side one going forward one going back
...
// Left turn with both side one going forward one going back
...
// Go forward with both sides running
...
// Go backwards with both sides running
...
// Set the right motor speed independently of the left, used by the PID engine for the line follower
...
// Set the left motor speed independently of the right, used by the PID engine for the line follower
...
void flashLED(int led, int times, int speed)
{		
...
void blinkLED(int led, int times, int speed)
{	
...
// Wait for a pin for a given timeout in micro-seconds to be either HIGH or LOW depending on the value of level 
...
void countPresses()
{
	...
	long micros = waitforpin(REDBUTTON,HIGH,30000000L);
	setLed(REDLED,0);
...
// ****************************************************************************
//
// Start for three point turn challenge
//
// ****************************************************************************
//
void resetCounters()
...
// Calculate the distance travelled using the ticks seen by the encoders
double distanceTravelled()
{
	//// Going to average the encoder counters for the moment
	//return((( rightCount + leftCount ) /2) * mmPerTick);
	// Change to using a single side
	return( rightCount * mmPerTick );
}

// Drive a target distance in the direction specified
bool drive(int target, Direction dir)
{
	resetCounters();
	...
	{
		if(getKey() == 'x' || digitalRead(REDBUTTON) == LOW)
			return false;		
    }
	Stop();
	return true;
...
// Turn to the left using the encoders
bool left(int distance)
{
	resetCounters();
	SharpLeft(lowSpeed );
...
void threePointTurn()
{
	flashLED(GRNLED, 2, 500 );
	runThreePointTurn = true;
	
	runThreePointTurn = runThreePointTurn && drive(1410, Direction::FORWARD);
	runThreePointTurn = runThreePointTurn && left(200);
	runThreePointTurn = runThreePointTurn && drive(600, Direction::FORWARD);
	runThreePointTurn = runThreePointTurn && drive(1200, Direction::BACKWARD);
	runThreePointTurn = runThreePointTurn && drive(600, Direction::FORWARD);
	runThreePointTurn = runThreePointTurn && left(200);
	runThreePointTurn = runThreePointTurn && drive(1400, Direction::FORWARD);

	flashLED(REDLED, 2, 500 );
...
// ****************************************************************************
//
// Start for the remote control challenges
//
// ****************************************************************************
//
...
{	
	...
	while( running )
	{
		...
		snapLeft = leftCount;
		snapRight = rightCount;
		
		// Calculate the distance travelled
		leftDistance = snapLeft * mmPerTick;
		rightDistance = snapRight * mmPerTick;
		
		if( prevLeft != snapLeft || prevRight != snapRight )
		{
			printf("Left distance travelled = %6.2fmm counter = %d\n", leftDistance, snapLeft);
			printf("Right distance travelled = %6.2fmm counter = %d\n", rightDistance, snapRight);
		}
		prevLeft = snapLeft;
		prevRight = snapRight;
...
// ****************************************************************************
//
// Start of the line follower challenge
//
// ****************************************************************************
...
void lineFollower()
{
	...
	while (running) 
	{
		// PID control of motors - feed back to the motors based on the rate of change of position.
...
// ****************************************************************************
//
// Start of the proximity challenge
//
// ****************************************************************************
...
State checkDistance( State state, double target )
{
...
{	
	...
	while (running) {
		...
		if( state == State::Moving )			
			state = checkDistance(state, target);
...
// Actual proximity challenge, advance to 300mm and check then advance to final target distance
void proximity()
{
	flashLED(GRNLED, 2, 500);
	runProximity = true;
...
// ****************************************************************************
//
// Main function - Setp Wiring Pi call backs and 
//                 run a menu to select individual challenges
//
// ****************************************************************************
int main(int argc, char** argv) 
{
	...
	pullUpDnControl (GRNBUTTON, PUD_UP);  
	...
	// Our encoders, sonar and IR sensor pins
	pinMode (ENCODER_LEFT, INPUT);
	//pullUpDnControl (ENCODER_LEFT, PUD_DOWN);
	pinMode (ENCODER_RIGHT, INPUT);
	//pullUpDnControl (ENCODER_RIGHT, PUD_DOWN);
	pinMode (SONAR, INPUT);
	//pullUpDnControl (SONAR, PUD_DOWN);
	pinMode (SENSOR, INPUT);
	//pullUpDnControl (SENSOR, PUD_UP);
	
	
	// Call backs must be done here as they can't be cancelled ( except by closing the application! )	
	// Bit of a weakness really.
	...
	// Do a little dance for our watchers to show we have started
	for (int i=0; i< 5; i++)
	{
		flashLED(GRNLED, 1, 200);
	...
	while (inMenu) 
	{	 
		blinkLED(GRNLED, presses, 200);  
	
		// Increment the counter, or reset it or launch the challenge
		if (digitalRead(REDBUTTON) == LOW)
			countPresses( );
			
		// Alternative launch when two buttons present
		if (digitalRead(GRNBUTTON) == LOW)
			launch = true;
	...
	// Say "good night" Lucy
	flashLED(REDLED, 2, 250 );
	flashLED(GRNLED, 2, 250 );
